...
def 材料定价计算():
    global 游戏物品数据, 游戏关卡数据, 定价关卡列表, 精英材料编号列表, 精英材料名列表, 精英材料图片列表, 其他物品信息列表
        ...
        if 游戏关卡数据['stages'][关卡]['difficulty'] == 'NORMAL':  # 不考虑突袭
            ...
            if 游戏关卡数据['stages'][关卡]['zoneId'] == 'weekly_6':
                ...
                if 游戏关卡数据['stages'][关卡]['zoneId'] in ['act10d5_zone1', 'act13d0_zone1', 'act10d5_zone1']\
                    ...
                    if 活动代币.get(): 折算产出龙门币 += 游戏关卡数据['stages'][关卡]['apCost'] * 10
                    ...
                    折算产出龙门币 = 游戏关卡数据['stages'][关卡]['goldGain'] * 1.2
                    if 活动代币.get(): 折算产出龙门币 += 游戏关卡数据['stages'][关卡]['apCost'] * 20
    ...
    if not 瑕光[1].get(): T4精英材料价值向量 += numpy.ones(len(精英材料编号列表[3])) * 300 * 龙门币价值
    T5精英材料价值向量 = -numpy.dot(T4精英材料价值向量, 加工T5获得T4精英材料矩阵)
    if not 瑕光[0].get(): T5精英材料价值向量 += numpy.ones(len(精英材料编号列表[4])) * 400 * 龙门币价值
    T2精英材料价值向量 = -numpy.dot(numpy.dot(T3精英材料价值向量, 加工T3获得T3精英材料矩阵), numpy.linalg.inv(加工T3获得T2精英材料矩阵))
    if not 瑕光[2].get(): T2精英材料价值向量 += numpy.dot(numpy.ones(len(精英材料编号列表[1])) * 200 * 龙门币价值, numpy.linalg.inv(加工T3获得T2精英材料矩阵))
    ...
    if not 瑕光[2].get(): T1精英材料价值向量 -= 200 * 龙门币价值 * numpy.dot(numpy.dot(numpy.ones(len(精英材料编号列表[1])), numpy.linalg.inv(加工T3获得T2精英材料矩阵)), numpy.linalg.inv(加工T2获得T1精英材料矩阵))
    if not 瑕光[3].get(): T1精英材料价值向量 += 100 * 龙门币价值 * numpy.dot(numpy.ones(len(精英材料编号列表[0])), numpy.linalg.inv(加工T2获得T1精英材料矩阵))
    ...
    for 序号, 材料名 in enumerate(精英材料名列表[2]):
        ...
        界面.Label(定价关卡域, text=format(T3精英材料价值向量[序号], '.2f')).grid(row=序号+1, column=2, padx=10, pady=2)
...
界面.Button(滚动区域[0], text="开 始 计 算", bootstyle=(SECONDARY), command=材料定价计算).grid(row=1, rowspan=3, column=0, padx=10, ipadx=200, ipady=20, sticky=界面.W+E+N+S)
